...
- **RSS 抓取稳健性**：
  ...
  - 为避免单次刷新过慢，每个 RSS 源仅处理最近 30 条 entries，在扩大候选池的同时保持单源超时与失败隔离。
- **Google Search 近 3 天热点模式**：改为通过 Serper API（优先 news 模式）获取近 3 天热点，再复用现有 AI 过滤、评分、摘要、topic 分类与多样性机制。候选面已扩大：每个 query 的抓取条数提升，并扩展了多模态、扩散、Agent、动画/VFX、3D 生成、创作者工具、AI 叙事等查询词。
...
- **手动刷新可见变化优化**：
  - 系统会记录“当天 + 当前模式”最近 3-5 轮展示历史（URL 集合），不只依赖上一轮；
  - 手动刷新时会优先避开“上一次展示 + 最近展示历史”中的 URL，并优先换入更近发布时间且未展示过的高质量候选；
  - RSS 模式偏稳定：在候选足够时手动刷新尽量替换 2 条重复内容；
  - Search 模式偏探索：在候选足够时手动刷新尽量替换 3 条重复内容，并在同分/近分候选中轻微优先更近的新文章；
  - 若候选不足或替换会显著降低质量，则自动回退到原有高质量结果，不为变化牺牲质量。
