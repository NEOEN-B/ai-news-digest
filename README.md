...
- **手动刷新可见变化优化**：
  - 系统会记录“当天 + 当前模式”最近一次展示的 URL 列表；
  - RSS 模式在保证稳定性的前提下，会优先尝试补入未展示过的高质量候选；
  - Search 模式手动刷新时会优先换入 1-2 条未在上次展示中出现的高质量候选，并对同分候选做轻微轮换，减少“刷新后完全不变”的体感。
  - 若候选池不足，则自动回退到原有高质量结果，不为变化牺牲质量。
